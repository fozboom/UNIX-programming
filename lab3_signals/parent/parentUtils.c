...
void createChildProcess(void) {
    ...
    if (pid == -1) {
        ...
        createdChildProcessesPids[countCreatedChildProcesses].allowedToPrintStatistics = true;
...
void printMenu() {
    printf("[+] - создать дочерний процесс\n"
           ...
           "[p] <num> - показывать статистику C_{num} и запретить другим процессам показывать статистику\n"
           "[q] - выход\n"
           "---> ");
}
...
void processCreateProcessCommand() {
    printf("\nCommand '+' was entered\n");
    createChildProcess();
}

void processDeleteLastProcessCommand() {
    printf("\nCommand '-' was entered\n");
...
void processDeleteAllProcessesCommand() {
...
void processDisableShowStatsCommand(UserInput userInput) {
    if (userInput.argument != -1) {
        ...
        sendSignalToBlockStatisticsOutput(createdChildProcessesPids[userInput.argument]);
    } else {
...
void processEnableShowStatsCommand(UserInput userInput) {
    if (userInput.argument != -1) {
        ...
        sendSignalToAllowStatisticsOutput(createdChildProcessesPids[userInput.argument]);
    } else {
...
void parentHandler(int signal, siginfo_t *siginfo, void *context)
{
    ...
    {
        printf("Segmentation ffkdfjdkfjkdfault\n");
        deleteAllChildProcesses();
        exit(EXIT_FAILURE);
    }
}
